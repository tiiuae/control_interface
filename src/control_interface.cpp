...
private:
  ...
  rclcpp::Subscription<nav_msgs::msg::Path>::SharedPtr                  waypoints_subscriber_;
  ...
  void waypointsCallback(const nav_msgs::msg::Path::UniquePtr msg);
...
ControlInterface::ControlInterface(rclcpp::NodeOptions options) : Node("control_interface", options) {
  ...
  waypoints_subscriber_ = this->create_subscription<nav_msgs::msg::Path>("~/waypoints_in", 10, std::bind(&ControlInterface::waypointsCallback, this, _1));
...
void ControlInterface::waypointsCallback(const nav_msgs::msg::Path::UniquePtr msg) {
  if (!abortMission()) {
    RCLCPP_WARN(this->get_logger(), "[%s]: Setpoint not set, previous mission cannot be aborted", this->get_name());
    return;
  }

  RCLCPP_INFO(this->get_logger(), "[%s]: Got %d waypoints", this->get_name(), msg->poses.size());

...
private:
  ...
  bool takeoffCallback(
      ...
      std::shared_ptr<std_srvs::srv::SetBool::Response> response);
  bool
  landCallback(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
               std::shared_ptr<std_srvs::srv::SetBool::Response> response);
  bool localSetpointCallback(
      const std::shared_ptr<fog_msgs::srv::Vec4::Request> request,
      std::shared_ptr<fog_msgs::srv::Vec4::Response> response);
  bool setWaypointsCallback(
  ...
  geometry_msgs::msg::PoseStamped transformBetween(std::string frame_from,
                                                   std::string frame_to);
...
ControlInterface::ControlInterface(rclcpp::NodeOptions options)
  ...
  vehicle_command_publisher_ =
      this->create_publisher<px4_msgs::msg::VehicleCommand>(
          "~/vehicle_command_out", 10);
  local_odom_publisher_ =
      this->create_publisher<nav_msgs::msg::Odometry>("~/local_odom_out", 10);
  marker_publisher_ =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(
          "~/debug_markers_out", 10);
  ...
  gps_subscriber_ =
      this->create_subscription<px4_msgs::msg::VehicleGlobalPosition>(
          "~/gps_in", 10, std::bind(&ControlInterface::gpsCallback, this, _1));
  pixhawk_odom_subscriber_ =
  ...
  arming_service_ = this->create_service<std_srvs::srv::SetBool>(
      "~/arming_in",
      std::bind(&ControlInterface::armingCallback, this, _1, _2));
  ...
  local_setpoint_service_ = this->create_service<fog_msgs::srv::Vec4>(
      "~/local_setpoint_in",
      std::bind(&ControlInterface::localSetpointCallback, this, _1, _2));
  set_waypoints_service_ = this->create_service<fog_msgs::srv::Path>(
      "~/waypoints_in",
      std::bind(&ControlInterface::setWaypointsCallback, this, _1, _2));
...
/* timesyncCallback //{ */
void ControlInterface::timesyncCallback(
    const px4_msgs::msg::Timesync::UniquePtr msg) {
  if (!is_initialized_) {
    return;
  }
...
/* gpsCallback //{ */
void ControlInterface::gpsCallback(
    const px4_msgs::msg::VehicleGlobalPosition::UniquePtr msg) {
  if (!is_initialized_) {
    return;
  }
  if (!getting_gps_) {
...
/* pixhawkOdomCallback //{ */
void ControlInterface::pixhawkOdomCallback(
    const px4_msgs::msg::VehicleOdometry::UniquePtr msg) {
  if (!is_initialized_) {
    return;
  }
  pos_.x() = msg->x;
  ...
  RCLCPP_INFO_ONCE(this->get_logger(), "[%s]: Getting pixhawk odometry!",
                   this->get_name());
...
bool ControlInterface::setWaypointsCallback(
    const std::shared_ptr<fog_msgs::srv::Path::Request> request,
    std::shared_ptr<fog_msgs::srv::Path::Response> response) {

  if (!abortMission()) {
    RCLCPP_WARN(this->get_logger(),
                "[%s]: Setpoint not set, previous mission cannot be aborted",
                this->get_name());
...
bool ControlInterface::takeoffCallback(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::SetBool::Request>
        request,
    std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  if (is_initialized_ && armed_ && getting_pixhawk_odom_ && getting_gps_ &&
      !airborne_) {
...
bool ControlInterface::localSetpointCallback(
    const std::shared_ptr<fog_msgs::srv::Vec4::Request> request,
    std::shared_ptr<fog_msgs::srv::Vec4::Response> response) {
  if (!abortMission()) {
    response->message = "Setpoint not set, previous mission cannot be aborted";
...
void ControlInterface::controlRoutine(void) {
  ...
  if (is_initialized_ && getting_gps_ && getting_pixhawk_odom_) {
    ...
    if (takeoff_requested_) {
      ...
      if (!armed_) {
        RCLCPP_INFO(this->get_logger(), "[%s]: Vehicle not armed",
                    this->get_name());
        return;
      }
      ...
      if (!airborne_) {
        while (!takeoff()) {
          RCLCPP_INFO_ONCE(this->get_logger(), "[%s]: Attempting to takeoff",
                           this->get_name());
    ...
    if (motion_started_) {
      ...
      if (waypoint_buffer_.size() < 1) {
        ...
        auto pos_in_world_tf = tf_buffer_->lookupTransform(
            world_frame_, fcu_frame_, rclcpp::Time(0));
        Eigen::Vector3d pos_in_world(pos_in_world_tf.transform.translation.x,
                                     pos_in_world_tf.transform.translation.y,
                                     pos_in_world_tf.transform.translation.z);

        if ((pos_in_world - current_goal_).norm() < 0.4) {
          // current goal reached
          RCLCPP_INFO(this->get_logger(), "[%s]: Moving to the next waypoint",
                      this->get_name());
    ...
    RCLCPP_INFO(this->get_logger(),
                "[%s]: Initialized: %s, GPS: %s, PX4 Odom: %s",
                this->get_name(), is_initialized_ ? "TRUE" : "FALSE",
                getting_gps_ ? "TRUE" : "FALSE",
                getting_pixhawk_odom_ ? "TRUE" : "FALSE");
  }
}
//}
...
bool ControlInterface::takeoff() {
  ...
  if (result != mavsdk::Action::Result::Success) {
    RCLCPP_WARN(this->get_logger(), "[%s]: Failed to set takeoff height %.2f",
...
bool ControlInterface::startMission() {
  ...
  if (result != mavsdk::Mission::Result::Success) {
    RCLCPP_WARN(this->get_logger(), "[%s]: Mission start rejected",
                this->get_name());
    return false;
  }
...
bool ControlInterface::uploadMission() {
  ...
  if (result != mavsdk::Mission::Result::Success) {
    RCLCPP_WARN(this->get_logger(), "[%s]: Mission upload failed",
                this->get_name());
    return false;
  }
...
bool ControlInterface::abortMission() {
  ...
  if (result != mavsdk::Mission::Result::Success ||
    ...
    RCLCPP_WARN(this->get_logger(), "[%s]: Mission cannot be aborted",
                this->get_name());
    return false;
  }
...
void ControlInterface::addGlobalToMission(Eigen::Vector3d waypoint) {
  ...
  RCLCPP_INFO(this->get_logger(),
              "[%s]: Waypoint (GPS) [%.2f,%.2f,%.2f,%.2f] added into mission",
...
ControlInterface::transformBetween(std::string frame_from,
                                   std::string frame_to) {
  geometry_msgs::msg::PoseStamped pose_out;
  try {
    auto transform_stamped =
        tf_buffer_->lookupTransform(frame_to, frame_from, rclcpp::Time(0));
...
/* parse_param impl //{ */
template bool ControlInterface::parse_param<int>(std::string param_name,
                                                 int &param_dest);
template bool ControlInterface::parse_param<double>(std::string param_name,
                                                    double &param_dest);
template bool ControlInterface::parse_param<float>(std::string param_name,
                                                   float &param_dest);
template bool
ControlInterface::parse_param<std::string>(std::string param_name,
                                           std::string &param_dest);
template bool
ControlInterface::parse_param<unsigned int>(std::string param_name,
